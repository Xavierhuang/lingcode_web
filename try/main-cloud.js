...
DATA — window.lingcode.from('<table>'):
  ...
  INSERT:  await window.lingcode.from('todos').insert({ title:'Buy milk' });   // single row, or .insert([ {…}, {…} ]) to batch many in ONE transaction (up to the tier's maxRowsPerWrite)
  UPSERT (insert-or-update by a unique key):  await window.lingcode.from('events').upsert(rows, { onConflict:'source_hash' });  // INSERT ... ON CONFLICT DO UPDATE; merge:false → DO NOTHING. The conflict column needs a unique/PK constraint (add via apply_migration).
  ...
  RPC (complex reads — JOINs, CTEs, aggregates, full-text ts_rank):  const { data } = await window.lingcode.rpc('search_events', { q:'jazz', in_city:'NYC' });  // calls a SQL function you create in apply_migration (CREATE FUNCTION); SQL stays server-side, runs under RLS, returns up to 1000 rows. Named-args object or positional array.
...
LIMITS — the builder (.from) is single-table, so write code accordingly: the .select() builder does NO JOINs — for a relational/aggregate/full-text read, write a SQL function in apply_migration and call it with .rpc() (or make a VIEW and select from it); batch insert/upsert is capped at the tier's maxRowsPerWrite per call (split larger loads); .select() returns at most 200 rows — page with .limit()/.offset() (.rpc() returns up to 1000). Upserts ARE supported (.upsert with onConflict) — no fetch-then-update dance needed. Schema (tables, indexes, constraints, views, functions, seed data) goes in apply_migration (full Postgres SQL), NOT in app-runtime calls.
